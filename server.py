...
def md2pptx_content_generator(
    ...
    为大模型提供生成符合 md2pptx 模板要求的 Markdown 内容的详细指导。
    确保生成的内容完全兼容 md2pptx 的格式要求和模板约束。
    """
    ...
    return f"""# 🎯 md2pptx 内容生成指导

## 📋 任务概述
**主题**: {topic}
**演示类型**: {presentation_type.title()}
**目标受众**: {audience}
**幻灯片数量**: {structure['slides']} 张
**内容深度**: {structure['depth']}

## 🚨 md2pptx 格式要求 (必须严格遵循)

### ⚡ 关键约束
1. **元数据头部必需**: 文件开头必须包含元数据，以空行结束
2. **标题层次严格**: # → 标题页, ## → 章节页, ### → 内容页, #### → 卡片
3. **模板引用必需**: 必须指定 `template: Martin Template.pptx`
4. **内容适量**: 每页要点 3-6 个，避免过长
...
### 📊 {presentation_type.title()} 演示框架

#### 🎯 内容重点
- **核心焦点**: {config['focus']}
- **表达风格**: {config['tone']}
- **章节数量**: {structure['sections']} 个主要章节
...
请根据以上指导，为主题 "{topic}" 生成一个完整的 md2pptx 兼容的 Markdown 演示文稿，要求：

1. **严格遵循 md2pptx 格式要求**
2. **包含完整的元数据头部**
3. **使用正确的标题层次结构**
4. **内容适合 {audience} 受众**
5. **体现 {config['tone']} 的表达风格**
6. **重点关注 {config['focus']}**

### 📝 具体要求
- 生成 {structure['slides']} 张幻灯片
- 包含 {structure['sections']} 个主要章节
- 每页要点控制在 3-6 个
- 适当使用表格、卡片等高级功能
- 确保内容逻辑清晰、层次分明

### 🎯 输出格式
直接输出完整的 Markdown 代码，可以直接用于 md2pptx 转换。
